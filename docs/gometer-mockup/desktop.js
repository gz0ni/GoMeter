/* ===== GoMeter Desktop · логика мокапа (Material 3) ===== */
...
document.querySelectorAll("[data-theme]").forEach((chip) => {
  chip.addEventListener("click", () => { state.theme = chip.dataset.theme; applyState(); });
});
...
function showScreen(name) {
  ...
  document.querySelector(".desktop-content").classList.toggle("is-onboarding", name === "onboarding");
  document.querySelector(".desktop-content").scrollTop = 0;
}
...
const railToggleIcon = document.getElementById("rail-toggle-icon");

function setRailExpanded(expanded) {
  rail.classList.toggle("is-expanded", expanded);
  railToggleIcon.textContent = expanded ? "chevron_left" : "chevron_right";
  railToggle.setAttribute("aria-label", expanded ? "Свернуть меню" : "Раскрыть меню");
  try { localStorage.setItem("gometer.railExpanded", expanded ? "1" : "0"); } catch (e) {}
}
railToggle.addEventListener("click", () => {
  setRailExpanded(!rail.classList.contains("is-expanded"));
});
setRailExpanded(localStorage.getItem("gometer.railExpanded") === "1");
