...
Future<int> _package(
  ...
  final distributorTargets = targets.split(',').where((t) => t != 'tar.gz').join(',');
  ...
  if (platform == 'linux' && targets.contains('tar.gz')) {
    final version = await _readVersion(rootDir);
    final linuxDir = arch == 'amd64' ? 'x64' : 'arm64';
    final bundleDir = p.join(rootDir, 'build', 'linux', linuxDir, 'release', 'bundle');
    final tarName = 'GoMeter-$version-linux-$arch.tar.gz';
    final tarPath = p.join(rootDir, 'dist', tarName);
    stdout.writeln('Creating $tarName ...');
    final tarResult = await Process.run('tar', [
      '-czf', tarPath,
      '-C', p.dirname(bundleDir),
      p.basename(bundleDir),
    ]);
    if (tarResult.exitCode != 0) {
      stderr.write(tarResult.stderr);
      return tarResult.exitCode;
    }
  }
...
Future<bool> _hasCommand(String cmd) async {
  ...
  final result = await Process.run('sh', ['-c', 'command -v $cmd']);
  return result.exitCode == 0;
}
